...
double  calcualteGCContent(char *sequence) {
...
int main() {
  ...
  printf("Enter the no of Genetic Sequences:");
  ...
  for(int i=0;i<numSequences;i++) {
    ...
    gcContent[i] = 0.0;
    printf("Enter the Genetic Sequence %d:",i+1);
    scanf("%s",sequences[i]);
    countNucleotides(sequences[i],nucleotideCounts[i]);
    gcContent[i] = calcualteGCContent(sequences[i]);
  }
  for(int i=0;i<numSequences;i++) {
    printf("Sequence %d:\n",i+1);
